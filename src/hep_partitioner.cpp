...
size_t HepPartitioner::count_mirrors()
{
    ...
    double avg_vertice_cnt = all_part_vertice_cnt / p;
...
void HepPartitioner::calculate_stats()
{
    ...
    for (int b = 0; b < p; ++ b) {
        max_part_vertice_cnt = std::max(max_part_vertice_cnt, get_boundary()[b].popcount());
        all_part_vertice_cnt += get_boundary()[b].popcount();
    ...
    for (int b = 0; b < p; ++ b) 
        std_deviation += pow(get_boundary()[b].popcount() - avg_vertice_cnt, 2);
    std_deviation = sqrt((double)std_deviation / p);
