...
if [ $partition_needed -eq 1 ]; then
    # Partition SMP
    for ((k = 2; k <= 4; ++k))
    do
        logname=$filename"_smp_k_"$k".log"
        echo $logname
        nohup ../release/main -p $p -k $k -method smp -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
        pid=$!
        echo $pid
        wait
    done
    ...
    do
        ...
        echo $logname
        nohup ../release/main -p $p -hdf $hdf -method hep -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
        pid=$!
        echo $pid
        wait
    done
    ...
    echo $logname
    nohup ../release/main -p $p -method ne -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
    pid=$!
    echo $pid
    wait
    ...
    echo $logname
    nohup ../release/main -p $p -method ebv -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
    pid=$!
    echo $pid
    wait
    ...
    echo $logname
    nohup ../release/main -p $p -method hdrf -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
    pid=$!
    echo $pid
    wait
    ...
    echo $logname
    nohup ../release/main -p $p -method dbh -filename "../../dataset/"$filename -write true > $partition_info_path/p_$p/$short/$logname &
    pid=$!
    echo $pid
    wait
fi
